...
$passengerquery = "SELECT id, p_lname, t_no FROM passengers WHERE t_no='$row[t_no]';";
$passengerresult = mysqli_query($conn, $passengerquery);
$passengerrow = mysqli_fetch_assoc($passengerresult);
if(strlen($passengerrow[p_lname]) > 3)
	$pnr_generated = $passengerrow[id].substr($passengerrow[p_lname], 0, 3).$passengerrow[t_no];
else
	$pnr_generated = $passengerrow[id].$passengerrow[p_lname].$passengerrow[t_no];
...
{  
	$pnr = "INSERT INTO tickets (t_status, PNR) VALUES ('CONFIRM', '$pnr_generated');";
	if(mysqli_query($conn, $pnr))
		$message = "Ticket booked successfully and PNR is generated";
	else
		$message = "Ticket booked successfully but couldnt generate PNR";
}
	else {
